...
global_accounts = [
    '567d6a1f2d5bafae059308d91cb8a5452cb48032bed42a9ff5a601a7bab5e183',
    'd9fcf0b7789ebc007ab87acddaad50687973f9c065b1f1e0d43330d6d150fbc3',
    'c37421b012240d8f320eacf5d9bd378409ecbf8536b9bdeb85f45e94dd106c22',
    '92d12f7ff94ccecf1f5f61fc0b26b429ba9405bf1acb9ced1c3e9168d9cf6c81',
    ...
    '5b659920b75435d91c35ce9c171ea7117218c08585bde2497922437da0bc2e13',
...
def on_new_tx(tx):
    if tx['type'] == 0:
        ...
        # judge me
        account_me = accounts[(sender_index + 1) % len(global_accounts)]
        check_nonce(account_me)
        og_solver.send_tx(account_me, [tx['data']['hash']], account_me.nonce, account_me.address, 0,
                          account_me.public_key)
        account_me.nonce += 1
    elif tx['type'] == 1:
        print('Received Seq: ', tx)
        account_me = accounts[9]
        check_nonce(account_me)
        og_solver.send_tx(account_me, [tx['data']['hash']], account_me.nonce, account_me.address, 0,
                          account_me.public_key)
